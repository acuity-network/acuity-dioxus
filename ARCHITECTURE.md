...
| `Reactions` | `item_view/reactions.rs` | Emoji reaction chips with picker; loads reaction state from indexer `SetReactions` events using the exact composite `item_id_revision_id` key so only the selected revision is queried; submits `set_reactions` extrinsic with the full emoji set; uses optimistic local update after tx submission, reverts on failure. The indexer currently emits the `reactions` field as either a scalar string/number for a single emoji or an array for multiple emojis, so the UI parser accepts both shapes. |
...
| `ContentReactions` | Stateless emoji reactions via `set_reactions` extrinsic; reaction state loaded from indexer `SetReactions` events queried by the composite `item_id_revision_id` key. The decoded `reactions` field may arrive as a scalar value when only one emoji is present, so clients must not assume an array-only payload shape. |
